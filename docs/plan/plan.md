...
- [ ] **Phase 6: Deploy pipeline to a Rackspace Spot cluster via GitOps; build the Argo Workflow that deploys `web/` to Cloudflare Pages**
  Delivers: both halves live in production — pipeline running unattended on its chosen cluster, plus a new Argo WorkflowTemplate (`jedarden/declarative-config`, `k8s/iad-ci/argo-workflows/`, modeled on `website-build`) that runs `wrangler pages deploy` against `web/`'s built artifacts (see ADR-6) — not Cloudflare's git-integration auto-build.
  Completion criteria:
  - Pipeline completes at least 3 consecutive scheduled runs without manual intervention
  - The new WorkflowTemplate exists in `declarative-config`, synced by ArgoCD, triggered by an Argo Events webhook on push (or manual submission as the fallback, matching jedarden.com's pattern)
  - A workflow run successfully deploys `web/` via `wrangler pages deploy` and the live site loads the real Parquet file end-to-end
